...
/// a collection of products in a single HTTP request and dispatching them via CQRS handlers.
/// </summary>
public sealed class BatchController : ApiControllerBase
{
    ...
    public async Task<ActionResult<BatchCreateProductsResponse>> CreateProducts(
        ...
        [FromServices]
            ICommandHandler<BatchCreateProductsCommand, BatchCreateProductsResponse> handler,
        CancellationToken ct
    )
    {
        var result = await handler.HandleAsync(new BatchCreateProductsCommand(request), ct);
        return Ok(result);
    }
}
