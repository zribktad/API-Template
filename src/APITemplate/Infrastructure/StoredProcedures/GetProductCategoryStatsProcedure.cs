...
/// <summary>
/// Calls the <c>get_product_category_stats(p_category_id)</c> PostgreSQL function.
///
...
/// </summary>
public sealed record GetProductCategoryStatsProcedure(Guid CategoryId)
    : IStoredProcedure<ProductCategoryStats>
{
    public FormattableString ToSql() =>
        $"SELECT * FROM get_product_category_stats({CategoryId})";
}
