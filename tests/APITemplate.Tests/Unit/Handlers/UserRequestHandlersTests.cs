...
public class UserRequestHandlersTests
{
    ...
    public async Task GetByIdAsync_WhenUserExists_ReturnsUserResponse()
    {
        ...
        var sut = new GetUserByIdQueryHandler(_repositoryMock.Object);
        var result = await sut.HandleAsync(new GetUserByIdQuery(expected.Id), ct);
    ...
    public async Task CreateAsync_ReturnsCreatedUser()
    {
        ...
        var result = await sut.HandleAsync(new CreateUserCommand(request), ct);
        ...
        _publisherMock.Verify(
            p =>
                p.PublishAsync(
                    It.IsAny<UserRegisteredNotification>(),
                    It.IsAny<CancellationToken>()
                ),
            Times.Once
        );
    ...
    public async Task CreateAsync_WhenKeycloakFails_ThrowsAndDoesNotSaveUser()
    {
        ...
        await Should.ThrowAsync<HttpRequestException>(() =>
            sut.HandleAsync(new CreateUserCommand(request), ct)
        );
    ...
    public async Task UpdateAsync_WhenUserExists_UpdatesFields()
    {
        ...
        var sut = new UpdateUserCommandHandler(
            _repositoryMock.Object,
            _unitOfWorkMock.Object,
            _publisherMock.Object
        );
    ...
    public async Task ChangeRoleAsync_ChangesUserRole()
    {
        ...
        _publisherMock.Verify(
            p =>
                p.PublishAsync(
                    It.IsAny<UserRoleChangedNotification>(),
                    It.IsAny<CancellationToken>()
                ),
            Times.Once
        );
